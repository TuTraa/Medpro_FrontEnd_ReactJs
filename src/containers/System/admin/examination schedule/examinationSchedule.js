...
class ExaminationScheduleCancel extends Component {
    ...
    getDataPatient = async (doctorId, date, phone) => {
        ...
        if (selectStatusId === 'S0') {
            res = await (await getAllPatientForDoctorS0({ doctorId: doctorId, date: date, phone: phone })).data
        }
        ...
        if (selectStatusId === 'S2') {
            res = await (await getAllPatientForDoctorIsActive({ doctorId: doctorId, date: date, phone: phone })).data
        }
        if (selectStatusId === 'S3') {
            res = await (await getAllPatientForDoctorDone({ doctorId: doctorId, date: date, phone: phone })).data
        }
    ...
    handleChangeSlectStatus = async (selectedOption) => {
        ...
        let res = '';
        if (selectStatusId === 'S0') {
            res = await (await getAllPatientForDoctorS0({ doctorId: 'All', date: 'All', phone: '' })).data
        }
        if (selectStatusId === 'S1') {
            res = await (await getAllPatientForDoctorNotCome({ doctorId: 'All', date: 'All', phone: '' })).data
        }
        if (selectStatusId === 'S2') {
            res = await (await getAllPatientForDoctorIsActive({ doctorId: 'All', date: 'All', phone: '' })).data
        }
        if (selectStatusId === 'S3') {
            res = await (await getAllPatientForDoctorDone({ doctorId: 'All', date: 'All', phone: '' })).data
        }
        if (selectStatusId === 'S4') {
                ...
                >
                    <div className='manage-patient-container '>
                        <div className='m-p-title'>
                            <p style={{ color: '#0071ba', fontSize: '23px' }} >
                                {this.state.selectStatusId.label}
                            </p>
                        </div>
                                                ...
                                                {selectStatusId === 'S0'
                                                    ...
                                                    <td className='action-click-schedule'>
                                                        <div className='click-schedule'>
                                                            <i className="fas fa-check-square confirm " onClick={() => this.openConfirmModal(item.id)}></i>
                                                            <i className="fas fa-window-close cancel" onClick={() => this.openRefuseModal(item.id)}></i>
                                                    ...
                                                    <td className='action-click-schedule'>
                                                        <div className='click-schedule'>
                                                            {item.pay ?
                                                                <i className="fas fa-check paied"></i>
                                                                :
                                                                <i className="fas fa-money-check-alt pay" onClick={() => this.payBooking(item.id)}></i>
